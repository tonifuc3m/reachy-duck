...
from reachy_duck.memory import forget_memory_fact
...
class Forget(Tool):
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Failed to forget memory: %s", exc)
            return {"error": f"failed to forget memory: {exc}"}
        ...
        if len(result.candidates) > 1:
            response["other_matches"] = list(result.candidates[1:])

        logger.info("Tool call: forget query=%s removed=%s", query[:120], result.removed[:120])
        return response
