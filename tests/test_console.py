...
import reachy_duck.console as console_mod
from reachy_duck.config import HF_AVAILABLE_VOICES, config
from reachy_duck.console import LocalStream
from reachy_duck.startup_settings import (
    StartupSettings,
    load_startup_settings_into_runtime,
)
from reachy_duck.personality_routes import (
    RouteError,
...
def test_backend_startup_failure_is_recorded_without_raising(
    ...
    monkeypatch.setattr("reachy_duck.console.apply_audio_startup_config", MagicMock())
...
def test_media_warmup_overlaps_audio_startup_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("reachy_duck.console.has_hf_realtime_target", lambda: True)
    ...
    monkeypatch.setattr("reachy_duck.console.asyncio.sleep", wait_for_audio_config)
    monkeypatch.setattr("reachy_duck.console.apply_audio_startup_config", apply_audio_config)
...
def test_local_stream_persist_personality_clears_legacy_startup_env_overrides(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "reachy_duck.config.set_custom_profile",
...
def test_local_stream_launch_waits_for_missing_hf_target_without_starting_media(
    ...
    monkeypatch.setattr("reachy_duck.console.time.sleep", MagicMock(side_effect=KeyboardInterrupt))
