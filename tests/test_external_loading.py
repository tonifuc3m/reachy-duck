...
def _reload_core_tools() -> ModuleType:
    ...
    for module_name in list(sys.modules):
        if module_name.startswith(
            ("reachy_duck.tools.", "reachy_duck._external_tools.")
        ):
            sys.modules.pop(module_name, None)

    sys.modules.pop("reachy_duck.tools.core_tools", None)
    core_tools_mod = importlib.import_module("reachy_duck.tools.core_tools")
...
def test_external_tools_can_be_loaded_without_external_profile(
    ...
    assert "reachy_duck._external_tools.json" in sys.modules
